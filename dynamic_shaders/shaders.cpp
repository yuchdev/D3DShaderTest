...
private:
    ...
    void split(const std::string &txt, char ch = ' ')
    {
        ...
        while( pos != std::string::npos ) {
            m_parsedParams.push_back( txt.substr( initialPos, pos - initialPos ) );
            initialPos = pos + 1;
        ...
        // Add the last one
        m_parsedParams.push_back(txt.substr(initialPos, min(pos, txt.size()) - initialPos + 1));
    }
    std::vector<std::string> m_parsedParams;
};
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    LoadString(hInstance, IDC_SHADERS, ApplicationWindow::m_wndClass, MAX_LOADSTRING);
    ...
    // Main message loop
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_SHADERS));
    MSG msg;
    ...
    while( msg.message!=WM_QUIT )
    {
        ...
        else
        {
            ...
            VertPosDiffuse v[] = 
            {
                ...
                VertPosDiffuse(D3DXVECTOR3(1,  -1, 0), D3DCOLOR_XRGB(0, 0, 255)),
            ...
            D3DXMatrixLookAtLH(&matView, &D3DXVECTOR3(0, 2, 2), &D3DXVECTOR3(0,0,0), &D3DXVECTOR3(0, 1, 0));
...
ATOM ApplicationWindow::MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_SHADERS));
